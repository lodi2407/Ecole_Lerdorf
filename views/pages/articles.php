<div class="container">
    <?php if (isset($_SESSION['user']) && ($_SESSION['user']['nom_user'] === 'Administrateur' || 'enseignants')) : ?>
        ...
        <table class="tableArticles mb-3 mx-auto text-justify <?= (isset($_SESSION['user']) && ($_SESSION['user']['nom_user'] === 'Administrateur' || 'enseignants')) ? 'col-lg-10 col-md-11' : 'col-lg-8 col-md-10' ?>">
            <?php foreach($articles as $article) : ?>
                <tr>
                    ...
                    <th colspan="2">
                        <a href="/article/index/<?= $article['num_article'] ?>">
                            <h5 class="card-title articleTitle"><?= $article['titre_article']; ?></h5>
                        </a>
                    </th>
                    <?php if(isset($_SESSION['user']) && ($_SESSION['user']['nom_user'] === 'Administrateur' || 'enseignants')) { ?>  
                        <th class="thUpDel text-right" rowspan="2" valign="top">
                            ...
                            <!-- Fenêtre modale suppression article -->
                ...
                <tr>
                    ...
                    <td>
                        <p class="card-text"><?= $article['contenu_article']; ?>...
                            <a href="/article/index/<?= $article['num_article'] ?>">Lire la suite</a>
                        </p>
                        <p class="text-right"><?= $article['auteur_article']; ?></p>
                        <p class="text-right"><small class="text-muted"><?= $article['date_article']; ?></small></p>
                    </td>
                </tr>
            <?php endforeach; ?>
        </table>
